...
def _dnaconvert(dna_input: str, dna_output: str, format: str) -> None:
    ...
    with open(dna_input, errors="surrogateescape") as dna_infile, open(
        dna_output, mode="w", errors="surrogateescape"
    ) as dna_outfile:
        convertDNA(
            dna_infile,
            dna_outfile,
...
def process(infile: TextIO, outfile: TextIO, format: str) -> None:
    with tempfile.TemporaryDirectory(prefix="dnaconvert_") as dir:
        ...
        with open(dna_input, mode="w", errors="surrogateescape") as dna_infile:
            for line in infile:
                dna_infile.write(line)

        _dnaconvert(dna_input, dna_output, format)

        with open(dna_output, errors="surrogateescape") as dna_outfile:
